...
Upload.uploadFile = async (req, res) => {
	if (req.files || req.body.title != "") {
		...
		let thumbnail: any = req.files.thumbnail;
		let thumbnail_autocreate: boolean = true;
		let img_ext: string = ".png";
		let ext: string = path.extname(file.name);
		let uniqid_inst: string = uniqid();
		let filename: string = uniqid_inst + ext;
		let thumbnail_name = uniqid_inst;
		let description: string = lib.replaceURLs(req.body.desc);
		...
		file.mv(__dirname + `/public/assets/media/${filename}`, async function (err: Error) {
			...
			if (thumbnail !== undefined) {
				...
				if (thumbnail.mimetype.includes("image/")) {
					...
					thumbnail.mv(
						__dirname + `/public/assets/images/${thumbnail_name + img_ext}`,
						async (err: Error) => {
							if (err) {
