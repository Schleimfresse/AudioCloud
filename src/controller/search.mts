...
Search.history = (req, res) => {
	...
	lib.searchhistory_database.findOne({ _id: data.user }, {}, (err: Error, user_history: any) => {
		if (user_history !== null) {
			user_history.history.forEach((e: { value: string; }) => {
		...
		if (user_history == null || user_history == "") {
			...
			if (user_history.history.length >= 8) {
				lib.searchhistory_database.update(
					{ _id: data.user },
					{ $pop: { history: -1 } },
					{},
					(err: Error, count: any) => {
...
Search.historydelete = (req, res) => {
	...
	lib.searchhistory_database.update(
		{ _id: req.body.user },
		{ $pull: { history: { value: req.body.value } } }
	);
	lib.searchhistory_database.loadDatabase();
};
...
Search.historydeleteall = (req, res) => {
	...
	lib.searchhistory_database.loadDatabase();
	lib.searchhistory_database.remove({ _id: req.body.user }, {}, (err: Error, count: number) => {
		if (err) {
